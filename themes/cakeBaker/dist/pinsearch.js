$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (
          ...
          countryCity.toUpperCase() != "Select Delivery City".toUpperCase()
        ) {
          ...
          if (countryCity.indexOf(",") > -1) {
            currentInternationalCity =
              jumpTo +
              "?c=" +
              countryCity.substring(0, countryCity.indexOf(","));
          }
  ...
  if ($("#openCitySearchModal").length > 0) {
    function designChange() {
      var selectedCtyNameSuggestion = localStorage.getItem(
        "selectedCtyNameSuggestion"
      );
      var selectedPincodeSuggestion = localStorage.getItem(
        "selectedPincodeSuggestion"
      );
      $("#wrPinMsg").addClass("hide");
    ...
    if ($(".editLoc").length) {
      ...
      if (typeof sessionCity != "undefined" && sessionCity.length > 2) {
        ...
        let selectedCityPostal = sessionCity.substring(
          sessionCity.indexOf(",") + 1
        );
        ...
        if (
          selectedPincodeSuggestion === "true" ||
          selectedCtyNameSuggestion === "true"
        ) {
        ...
        if (
          (crossClicked === null ||
            crossClicked === "null" ||
            typeof crossClicked === "undefined" ||
            crossClicked === "false") &&
          (cityRemove === null ||
            cityRemove === "null" ||
            typeof cityRemove === "undefined" ||
            cityRemove === false ||
            cityRemove === "false") &&
          selectedCtyNameSuggestion === "true"
        ) {
    ...
    var selectedCountryCitySuggestion = localStorage.getItem(
      "selectedCountryCitySuggestion"
    );
    var selectedSuggestion = localStorage.getItem("selectedSuggestion");
    ...
    $(document).on("click", ".delLocClose", function () {
      ...
      $(document).on(
        "click",
        ".adobDynamicCityName a",
        adbCityLocationSearchMob
      );
    ...
    function getCountryId(clickedElement) {
      var getCountryName = clickedElement
        .find(".contryText")
        .text()
        .trim()
        .replace(/-/g, " ");
      ...
      .on("click", function () {
        ...
        $(".countrySpace")
          ...
          .each(function () {
            ...
            if (
              prevSelectedCountry !== selectedCountry.toLowerCase() &&
              $("#" + prevSelectedCountry).hasClass("hide")
            ) {
    ...
    if (checkCountry !== "INTERNATIONAL") {
      var defultCheck;
      defultCheck = document.querySelector(
        "input[type=radio][name=with-india]"
      );
      defultCheck.checked = true;
      $("#openCitySearchModal").modal({
        onOpenEnd: function () {
          $("#openCitySearchModal input[name=with-india]").prop(
            "checked",
            true
          );
          withinIndiaClick();
        ...
        onCloseEnd: function () {
          var modalCloseViaContinue = localStorage.getItem(
            "modalCloseViaContinue"
          );
          var selectedCtyNameSuggestion = localStorage.getItem(
            "selectedCtyNameSuggestion"
          );
          ...
          if (
            ...
            (selectedCtyNameSuggestion === "true" ||
              selectedPincodeSuggestion === "true") &&
            onEdit !== "Enter Delivery Pincode or Area" &&
            crossClicked !== "true" &&
            modalCloseViaContinue !== "true" &&
            viaCheckBtn === "viaCheck"
          ) {
            location.reload();
          }
          $("#openCitySearchModal input[name=outside-india]").prop(
            "checked",
            true
          );
          outSideIndiaClick();
      ...
      function outSideIndiaClick() {
        $("#replaceGlobeImage").attr(
          "src",
          "https://assets.winni.in/groot/2023/10/25/mobile/icon-ionic-ios-globe.png"
        );
        $("#replaceBuildingImage").attr(
          "src",
          "https://assets.winni.in/groot/2023/10/25/mobile/building.png"
        );
        ...
        $(".onEditCountryCity").text("Select Delivery City");
        let selectedCountry = localStorage.getItem("selectedCountryStore");
        let selectedCountryCitySugg = localStorage.getItem(
          "selectedCountryCitySuggestion"
        );
      ...
      var inputFieldInternational =
        document.querySelector("#countryCitySearch");
      ...
      if (
        selectedCountryCitySuggestion !== "" &&
        selectedCountryCitySuggestion !== undefined &&
        selectedCountryCitySuggestion !== null &&
        checkCountryText !== "Select Country"
      ) {
      ...
      function countryCitySuggestionSelected(suggestionClicked) {
        ...
        if (countryCityIdAb !== null) {
          $("#replaceBuildingImage").attr(
            "src",
            "https://assets.winni.in/groot/2023/11/02/red-building.png"
          );
      ...
      function clearCountryCitySearch() {
        ...
        $(".selectedCountryCity").css({
          "background-color": "rgba(243,127,10,1",
          "pointer-events": "auto",
        });
        $(".onEditCountryCity").text("Select Delivery City");
        $(".imgEditCountryCity").addClass("hide");
        ...
        $("#indiaCityByName")
            ...
            },
            {
              ...
              templates: {
                suggestion: Handlebars.compile(
                  '<div areaName={{areaName}} class="citySearchSuggestionList">{{areaName}}</div>'
                ),
                empty: [
          ...
          .on("typeahead:selected", function (e, suggestion) {
            ...
            localStorage.setItem(
              "selectedCityPostal",
              suggestion.adobeSearchSuggestedPostal
            );
            designChange();
            cityIndiaSelected();
          });
            ...
            },
            {
              ...
              templates: {
                suggestion: function (item) {
                  if (item.id === "-1") {
                    return (
                      '<div data-target="deliveryCountry" class=" checkTcp outSideCountry  modal-trigger" style="color:blue">' +
                      item.message +
                      "</div>"
                    );
                  } else if (item.id === "-2") {
                    ...
                    return (
                      '<div class="handlerCountrySearch">' +
                      item.nameWithStateCode +
                      "</div>"
                    );
                  }
                },
