...
use app\modules\storage\events\FileEvent;
use app\modules\storage\file\Item;
use app\modules\storage\filters\MediumThumbnail;
use app\modules\storage\filters\TinyCrop;
use app\modules\storage\helpers\Storage;
use app\modules\storage\jobs\ImageFilterJob;
use app\modules\storage\models\StorageFile;
use app\modules\storage\models\StorageFilter;
use app\modules\storage\models\StorageFolder;
 ...
 *
 * + {{app\modules\storage\filesystem\LocalStorage}} (Default system for the admin module)
 * + {{app\modules\storage\filesystem\S3}}
 *
 * As files, images and folders implement the same traits you can also read more about enhanced usage:
 *
 * + Querying Data with {{\app\modules\storage\storage\QueryTrait}}
 * + Where conditions {{\app\modules\storage\storage\QueryTrait::where()}}
 *
 * ## Handling Files
 *
 * First adding a new file to the Storage system using the {{\app\modules\storage\storage\BaseFileSystemStorage::addFile()}} method.
 *
 * ```php
 * Yii::$app->storage->addFile('/the/path/to/File.jpg', 'File.jpg', 0, 1);
 * ```
 *
 * The response of the add file method is an {{\app\modules\storage\file\Item}} Object.
 *
 ...
 * ### Handling Images
 *
 * An image object is always based on the {{\app\modules\storage\file\Item}} object and a {{app\modules\storage\base\Filter}}. In order to add an image you already need a fileId and filterId. If the filterId is 0, it means no additional filter will be applied.
 *
 * ```php
 * Yii::$app->storage->addImage(123, 0); // create an image from file object id 123 without filter.
 * ```
 *
 * The newly created image will return a {{app\modules\storage\image\Item}} object.
 *
...
abstract class BaseFileSystemStorage extends Component
{
    ...
    /**
     * @var boolean When enabled the storage component will try to recreated missing images when {{app\modules\storage\image\Item::getSource()}} of an
    ...
    /**
     * @var boolean When enabled, the filters in the {{app\modules\storage\storage\BaseFileSystemStorage::$queueFiltersList}} will be applied to the uploaded file if the file is an image. We
    ...
    /**
     * @var array If {{app\modules\storage\storage\BaseFileSystemStorage::$queueFilters}} is enabled, the following image filters will be processed. We recommend
     ...
     *
     * See {{\app\modules\storage\storage\QueryTrait::where}} for condition informations.
     *
     * @param array $args An array with conditions to match e.g. `['is_hidden' => 1, 'is_deleted' => 0]`.
     * @return \app\modules\storage\file\Iterator An iterator object containing all files found for the condition provided.
     */
    public function findFiles(array $args = [])
    {
        return (new \app\modules\storage\file\Query())->where($args)->all();
    }
     ...
     *
     * @param array $args An array with conditions to match e.g. `['is_hidden' => 1, 'is_deleted' => 0]`.
     * @return \app\modules\storage\file\Item The file item object.
     */
    public function findFile(array $args)
    {
        return (new \app\modules\storage\file\Query())->where($args)->one();
    }
     ...
     * @return \app\modules\storage\file\Item|boolean The file object or false if not found.
     */
    public function getFile($fileId)
    {
        return (new \app\modules\storage\file\Query())->findOne($fileId);
    }
     ...
     *
     * + {{app\modules\storage\helpers\Storage::uploadFromFiles}}
     * + {{app\modules\storage\helpers\Storage::uploadFromFileArray}}
     *
     ...
     *
     * @param array $args An array with conditions to match e.g. `['is_hidden' => 1, 'is_deleted' => 0]`.
     * @return \app\modules\storage\image\Iterator An iterator object containing all image found for the condition provided.
     */
    public function findImages(array $args = [])
    {
        return (new \app\modules\storage\image\Query())->where($args)->all();
    }
     ...
     *
     * @param array $args An array with conditions to match e.g. `['is_hidden' => 1, 'is_deleted' => 0]`.
     * @return \app\modules\storage\image\Item The file item object.
     */
    public function findImage(array $args = [])
    {
        return (new \app\modules\storage\image\Query())->where($args)->one();
    }
     ...
     * @return \app\modules\storage\image\Item|boolean The image object or false if not found.
     */
    public function getImage($imageId)
    {
        return (new \app\modules\storage\image\Query())->findOne($imageId);
    }
     ...
     *
     * An image object is always based on the {{\app\modules\storage\file\Item}} object and a {{app\modules\storage\base\Filter}}.
     *
     * ```php
     * Yii::$app->storage->addImage(123, 0); // create an image from file object id 123 without filter.
     * ```
     ...
     * @return bool|\app\modules\storage\image\Item|Exception Returns the item object, if an error happens and $throwException is off `false` is returned otherwhise an exception is thrown.
    ...
    public function addImage($fileId, $filterId = 0, $throwException = false)
    {
        try {
            ...
            $query = (new \app\modules\storage\image\Query())->where(['file_id' => $fileId, 'filter_id' => $filterId])->one();
     ...
     * @since 1.2.2.1
     * @return \app\modules\storage\models\StorageImage|false Returns the storage image model on success, otherwise false.
     */
     ...
     *
     * @param array $args An array with conditions to match e.g. `['is_hidden' => 1, 'is_deleted' => 0]`.
     * @return \app\modules\storage\folder\Iterator An iterator object containing all image found for the condition provided.
     */
    public function findFolders(array $args = [])
    {
        return (new \app\modules\storage\folder\Query())->where($args)->all();
    }
     ...
     *
     * @param array $args An array with conditions to match e.g. `['is_hidden' => 1, 'is_deleted' => 0]`.
     * @return \app\modules\storage\folder\Item The folder item object.
     */
    public function findFolder(array $args = [])
    {
        return (new \app\modules\storage\folder\Query())->where($args)->one();
    }
     ...
     * @return \app\modules\storage\folder\Item|boolean The folder object or false if not found.
     */
    public function getFolder($folderId)
    {
        return (new \app\modules\storage\folder\Query())->where(['id' => $folderId])->one();
    }
     ...
     *
     * @param integer $filterIdentifier The filter identifier to find use {{app\modules\storage\base\Filter::identifier()}} method.
