...
- (enh #1072): _BC Breaking_: Gridview `replaceTags` optimized to render content only if necessary using callbacks.
...
- (enh #1009): (BC Breaking) Enhancements to float header, footer, and page summary.
  - floating the header, footer, or page summary is possible and will use the CSS _sticky_ property supported in new browsers
...
- (bug #1005): (BC Breaking) Remove _requires_ dependency on `yii2-bootstrap4-dropdown` & `yii2-bootstrap5-dropdown` and move it to _suggests_.
