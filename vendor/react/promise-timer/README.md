...
can be used to _cancel_ operations that take _too long_.
...
- If the input `$promise` does not settle before `$time` seconds, _cancel_ the operation and reject the resulting promise with a [`TimeoutException`](#timeoutexception).

Internally, the given `$time` value will be used to start a timer that will
_cancel_ the pending operation once it triggers.
...
As discussed above, the [`timeout()`](#timeout) function will _cancel_ the
underlying operation if it takes _too long_.
...
- A described above, a common use involves resource cleanup and will then _reject_
  the `Promise`.
...
- As described above, a common use involves resource cleanup and will then _reject_
  the `Promise`.
  If the input `$promise` is being rejected, then the timeout will be aborted
  and the resulting promise will also be rejected.
- If the input `$promise` is still pending, then the timout will continue
...
This will abort the timer and _reject_ with a `RuntimeException`.
